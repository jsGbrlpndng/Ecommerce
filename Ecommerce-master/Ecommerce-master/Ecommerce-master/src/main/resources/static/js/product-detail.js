...
async function loadProductDetails(productId) {
    try {
        ...
        // Use product.description from backend if available, else fallback to mappedDetailedDesc
        const description = product.description && product.description.trim() ? product.description : mappedDetailedDesc;
        // Hide old description if it exists
        const oldDesc = document.getElementById("product-description");
        if (oldDesc) oldDesc.textContent = '';
        // Show and set new centered description
        const descBox = document.getElementById("product-description-centered");
        if (descBox) {
            descBox.style.display = description ? '' : 'none';
            descBox.textContent = description || '';
        }
        const stockElem = document.getElementById("product-stock");
        if (stockElem) stockElem.textContent = product.stock;
        const availElem = document.getElementById("product-availability");
        if (availElem) {
            availElem.textContent = product.stock > 0 ? "In Stock" : "Out of Stock";
            availElem.className = product.stock > 0
                ? "availability-status in-stock"
                : "availability-status out-of-stock";
        }
        const skuElem = document.getElementById("product-sku");
        if (skuElem) skuElem.textContent = `MM-${product.id.toString().padStart(6, '0')}`;
        ...
        if (productDetailsBox) {
            let html = '';
            html += `<div class='product-category' style="background:#f3eaff;border-radius:12px;padding:18px 28px;margin-bottom:18px;text-align:center;font-size:1.25rem;font-family:'Montserrat',serif;font-weight:700;letter-spacing:1px;color:#7c3aed;"><span style='color:#7c3aed;'>CATEGORY:</span> <span style='color:#444;font-weight:500;'>${(product.category || 'N/A').toUpperCase()}</span></div>`;
            if (product.brand) {
        ...
        if (shortDescBox) {
            let shortDesc = product.description && product.description.trim() ? product.description : mappedDetailedDesc;
